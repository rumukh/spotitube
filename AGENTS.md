...
> **A detached pinned worktree answers about itself — a real trap, but not the cause of this
> incident.** Inside one, `git status`, `git log`, `git rev-parse HEAD` and every file read report
> that worktree's commit and contents. Prefer `git show origin/master:<path>` when asking what is
> pushed.
>
> Do not misattribute the 2026-07-29 stale corrections to that trap. Coordinator checks explicitly
> ran from `C:\src\spotitube`, and every PowerShell tool call starts a fresh process, so it could not
> inherit `pin-build.ps1`'s temporary worktree directory. The measured cause was out-of-order
> cross-session delivery: a 1,000 ms instruction completed sending at 21:53:29, the reviewer
> reversed it at 22:12:16, and the implementation session received the obsolete instruction at
> 23:33:22. Shared-tree churn also made conversational state stale between messages. Both facts are
> independently true; inventing a checkout explanation would conceal the channel failure actually
> measured.
...
| What actually reaches logcat | **Stated precisely, because the blanket claim "no user text, URL or query" was false in two places and had been marked *Measured*.** What IS logged, deliberately: the canonical Spotify link on `INPUT`/`SUPERSEDED`, and the `videoId` — per the standing rule that *removing the readable copy while leaving the resolvable copy is not a privacy fix*, since the videoId resolves to the same recording anyway. What is NOT logged: the user's surrounding message text (only its length), the search query, and — since this fix — any track, artist or album **display name**. `MATCH` was carrying `picked="<candidate>"` and `spotify="<the user's own track>"` on every successful play. **The lesson is about the audit method**: the device check grepped for `uri=`, `query=` and message bodies, which cannot find a leak in a field nobody suspected. Re-verify by reading every `Log.` call site — there are eleven — not by grepping for known offenders. |
...
2. **Copy → open Spotitube → paste** — the measured zero-setup fallback in Signal today. The
   automatic clipboard card did **not** appear on the vivo; never promise that a copied link will
   appear by itself.
...
### Deferred onboarding truthfulness findings (non-gating until matcher release)
